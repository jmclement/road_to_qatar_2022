...
def SDGClassifier():
    ...
    SGD.fit(X_train_transformed, y_train_encoded)
    score_train_acc = SGD.score(X_train_transformed, y_train_encoded)
    score_test_acc = SGD.score(X_test_transformed, y_test_encoded)
    print(score_train_acc) #0.46
    print(score_test_acc) #0.40
    y_pred_SGD = SGD.predict(X_test_transformed)
    print(classification_report(y_test_encoded, y_pred_SGD))
    print(confusion_matrix(y_test_encoded, y_pred_SGD, labels=range(3)))
